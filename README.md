...
4. **看圖表** — 10 個 Streamlit 頁面組成的暗色系儀表板，從市場總覽、個股 K 線到 A/B 比較器一氣呵成。
...
| 🏛️ **市場總覽** | 大盤行情 | 板塊輪動、法人籌碼、漲跌/量爆/突破排行榜 |
| 📈 **個股分析** | K 線深入 | RSI/MACD/KD 副圖、RS 相對強弱、Beta、ATR 建倉計算器、加入交易籃 |
| 🔧 **策略建構器** | 訊號可視化 | 9 策略切換、進出場標記、策略專屬指標副圖 |
| 📊 **回測結果** | 報表 | 三層佈局（摘要 / 資金曲線 / 卡片+交易明細）|
| 🔍 **選股工具** | 多因子篩選 | 量比、價格區間、RSI、距均線、55 板塊動態分類 |
| ⚔️ **策略 vs 基準** | 多策略比較 | 5 種已驗證策略 vs 0050、還原權息開關 |
