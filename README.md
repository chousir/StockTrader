...
4. **看圖表** — 9 個 Streamlit 頁面組成的暗色系儀表板，從市場總覽、個股 K 線到策略對照一氣呵成。
...
| 🏛️ **市場總覽** | 大盤行情 | 板塊輪動 treemap、法人籌碼、漲跌/量爆/突破排行榜 |
| 📈 **個股分析** | K 線深入 | RSI/MACD/KD 副圖、RS 相對強弱、Beta、ATR 建倉計算器、依產業選股 |
| 🔻 **兩階段選股漏斗** | 選股入口 | 全市場/產業 → 粗篩 4 條件 → 5 策略精篩 → 候選清單（顯示命中策略 F+H+L）|
| 📊 **回測結果** | 單股單策略回測 | 5 策略切換、含 0050 基準對照、月度熱力圖 |
| 🔍 **選股工具** | 多因子打分 | 量比、價格區間、RSI、距均線、Alpha 全宇宙掃描、55 板塊動態分類 |
| ⚔️ **策略 vs 基準** | 多策略比較 | 5 策略 vs 0050 並排對照、還原權息開關、依產業選股 |
| 🏗️ **組合回測** | 產業輪動 | 月度 Rebalance、Top-N、市場過濾器、支援交易籃 |
| 📡 **訊號掃描** | 全市場掃描 | 5 策略 × 全宇宙當日進場訊號，漲幅/量爆/突破 tabs |
| 🔔 **訂閱中心** | 告警 + 選股 | 個股告警規則 + 每日策略訂閱 + 觸發紀錄 + Discord，一頁統一管理 |
